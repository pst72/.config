local M = {
  ...
  keys = {
    ...
    -- Actions/Toggles
    { "<leader>ar", "<cmd>Telescope resume<cr>", desc = "Telescope Resume Last Action " },
    { "<leader>at", "<cmd>Telescope treesitter<cr>", desc = "Treesitter Functions " },
    -- Git
  ...
  config = function()
    ...
    telescope.setup({
      extensions = {
        fzf = fzf_opts,
        ["ui-select"] = {
          require("telescope.themes").get_cursor({}), -- 'get_cursor, get_dropdown, get_ivy'
        },
        file_browser = {
          theme = "dropdown",
          preview = true,
