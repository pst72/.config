...
M._keys = {
  ...
  { "]b", "<cmd>bnext<cr>", desc = "Goto Previous Buffer" },
  { "<F5>", "<cmd>setlocal spell!<cr>", desc = "Toggle SpellCheck" },
  { "<F6>", "<cmd>nohlsearch <cr>", desc = "Toggle Highlight" },
  { "<leader>lr", vim.lsp.buf.rename, desc = "Rename", has = "rename" },
  { "<leader>x", "<cmd>bdelete!<cr>", desc = "Close Buffer" },
  { "<leader>q", "<cmd>quit<cr>", desc = "Close Neovim" },
  {
    "<leader>lR",
    function()
      require("inc_rename")
...
keymap("x", "K", ":m '<0<CR>gv=gv", opts)
keymap("v", "J", ":m '>+3<CR>gv=gv", opts)
keymap("v", "K", ":m '<0<CR>gv=gv", opts)
-- Resize with arrows
...
keymap("n", "<F9>", "<cmd>Jag float<CR>", { desc = "Run Code in Float" })
