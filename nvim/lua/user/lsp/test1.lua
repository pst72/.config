...
local on_attach = function(client, bufnr)
	...
	-- ───────────────────────────────────────────────── --
	buf_set_keymap("n", "<leader>lo", "<cmd>lua vim.diagnostic.open_float()<CR>", options)
	buf_set_keymap("n", "<leader>lq", "<cmd>lua vim.diagnostic.set_loclist({})<CR>", options)
	...
	buf_set_keymap("n", "<leader>lt", "<cmd>lua vim.lsp.buf.type_definition()<CR>", options)
	...
	buf_set_keymap("n", "<leader>h", "<Cmd>lua vim.lsp.buf.hover()<CR>", options)
	buf_set_keymap("n", "K", "<Cmd>lua vim.lsp.buf.hover()<CR>", options)
	-- using 'filipdutescu/renamer.nvim' for rename
	buf_set_keymap("n", "<leader>lr", "<cmd>lua vim.lsp.buf.rename()<CR>", options)
	buf_set_keymap("n", "gr", "<cmd>Telescope lsp_references<CR>", options)
	buf_set_keymap("n", "<leader>lf", "<cmd>lua vim.lsp.buf.format({ timeout_ms = 2000 })<CR>", options)

	buf_set_keymap("n", "<leader>la", "<cmd>lua vim.lsp.buf.code_action()<CR>", options)
	buf_set_keymap("x", "<leader>la", "<cmd>lua vim.lsp.buf.range_code_action()<CR>", options)
